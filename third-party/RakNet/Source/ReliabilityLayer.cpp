...
int RakNet::SplitPacketIndexComp( SplitPacketIndexType const &key, InternalPacket* const &data )
{
...
void ReliabilityLayer::InitializeVariables( void )
{
	...
	statistics.connectionStartTime = GetTime();
	splitPacketId = 0;
	messageNumber = 0;
					...
					if ( orderingListAtOrderingStream != 0)
					{
						while ( orderingListAtOrderingStream->Size() > 0 )
						{
							...
							while (count++ < size)
							{
								...
								{
									/*
									BitStream temp(orderingListAtOrderingStream->Peek()->data, BITS_TO_BYTES(orderingListAtOrderingStream->Peek()->dataBitLength), false);
									temp.IgnoreBits(8);
...
//-------------------------------------------------------------------------------------------------------
unsigned ReliabilityLayer::GenerateDatagram( BitStream *output, int MTUSize, bool *reliableDataSent, RakNetTimeNS time, PlayerID playerId, DataStructures::List<PluginInterface*> &messageHandlerList  )
{
	InternalPacket * internalPacket;
//	InternalPacket *temp;
	int maxDataBitSize;
...
//-------------------------------------------------------------------------------------------------------
InternalPacket* ReliabilityLayer::CreateInternalPacketFromBitStream( BitStream *bitStream, RakNetTimeNS time )
{
...
void ReliabilityLayer::SplitPacket( InternalPacket *internalPacket, int MTUSize )
{
	...
	if (sizeof( InternalPacket* ) * internalPacket->splitPacketCount < _RAK_MAX_STACK_ALLOCATION)
	{
