...
int RakNet::PlayerIDAndIndexComp( const PlayerID &key, const PlayerIDAndIndex &data )
{
...
bool RakPeer::Initialize( unsigned short maxConnections, unsigned short localPort, int _threadSleepTimer, const char *forceHostAddress )
{
	...
	if ( maximumNumberOfPeers == 0 )
	{
		...
		//for ( i = 0; i < remoteSystemListSize; i++ )
		{
			...
			remoteSystemList[ i ].reliabilityLayer.rakPeer = this;
			#ifndef NDEBUG
	...
	//	lastUserUpdateCycle = 0;
			...
			myPlayerId.binaryAddress = inet_addr( forceHostAddress );
		{
			...
			if ( isMainLoopThreadActive == false )
			{
				...
				processPacketsThreadHandle = ( HANDLE ) _beginthreadex( NULL, _RAK_MAX_STACK_ALLOCATION*2, UpdateNetworkLoop, this, 0, &ProcessPacketsThreadID );
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::Connect( const char* host, unsigned short remotePort, const char* passwordData, int passwordDataLength )
{
...
bool RakPeer::Send( const char *data, const int length, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast )
{
	...
	RakAssert( remoteSystemList && endThreads == false );
	...
	{		
		...
		if (router && GetIndexFromPlayerID(playerId)==UNASSIGNED_PLAYER_INDEX)
...
bool RakPeer::Send( BitStream& bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast )
{
	return Send((const char*)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), priority, reliability, orderingChannel, playerId, broadcast);
}
...
Packet* RakPeer::Receive( void )
{
	...
	while ( (unsigned char) packet->data[ 0 ] == ID_RPC_REPLY )
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
		...
		rakPeerMutexes[packetPool_Mutex].Unlock();
#endif
		if ( ( packet->bitSize >= BYTES_TO_BITS(1 + sizeof( RakNetTime )) ) &&
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::RPC( RPCID uniqueID, const char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp )
{
#ifndef NDEBUG
	...
	for (sendListIndex=0; sendListIndex < (unsigned)sendListSize; sendListIndex++)
	{
		...
		if (shiftTimestamp)
		{
			...
			outgoingBitStream.Write(GetTime());
		}
...
#pragma warning( disable : 4701 ) // warning C4701: local variable <variable name> may be used without having been initialized
#endif
bool RakPeer::RPC( RPCID uniqueID, BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp )
{
	if (bitStream)
...
PlayerID RakPeer::GetPlayerIDFromIndex( int index )
{
	...
	if ( (unsigned)index < maximumNumberOfPeers )
...
//		return;
	...
	BitStream bitStream( sizeof(unsigned char) + sizeof(RakNetTime) );
	...
	SocketLayer::Instance()->SendTo( this, connectionSocket, (const char*)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), ( char* ) host, remotePort );
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BitStream * RakPeer::GetRemoteStaticData( const PlayerID playerId )
{
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::SendConnectionRequest( const char* host, unsigned short remotePort, const char* passwordData, int passwordDataLength )
{
	PlayerID playerId;
	IPToPlayerID( host, remotePort, &playerId );
...
void RakPeer::ParseConnectionRequestPacket( RakPeer::RemoteSystemStruct *remoteSystem, PlayerID playerId, const char *data, int byteSize )
{
	...
	if ( !AllowIncomingConnections() )
	{
		...
		// SocketLayer::Instance()->SendTo( this, rakPeer->connectionSocket, ( char* ) & c, sizeof( char ), systemAddress.binaryAddress, systemAddress.port );
		SendImmediate(( char* ) & c, sizeof( char )*8, SYSTEM_PRIORITY, RELIABLE, 0, playerId, false, false, GetTime());
...
void RakPeer::OnConnectionRequest( RakPeer::RemoteSystemStruct *remoteSystem, unsigned char *AESKey, bool setAESKey )
{
	...
	//if ( AllowIncomingConnections() )
	{
		BitStream bitStream(sizeof(unsigned char)+sizeof(unsigned short)+sizeof(unsigned int)+sizeof(unsigned short)+sizeof(PlayerIndex));
...
//		bitStream.Write((unsigned short)myPlayerId.port);
		...
		SendImmediate((char*)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), SYSTEM_PRIORITY, RELIABLE, 0, remoteSystem->playerId, false, false, GetTime());
	...
	{
		unsigned char c = ID_NO_FREE_INCOMING_CONNECTIONS;
		//SocketLayer::Instance()->SendTo( this, connectionSocket, ( char* ) & c, sizeof( char ), playerId.binaryAddress, playerId.port );

		SendImmediate((char*)&c, sizeof(c)*8, SYSTEM_PRIORITY, RELIABLE, 0, remoteSystem->systemAddress, false, false, GetTime());
		remoteSystem->connectMode=RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY;
	}
	*/
}
...
{
	BitStream temp( sizeof(unsigned char) );
	temp.Write( (unsigned char) ID_DISCONNECTION_NOTIFICATION );
	if (performImmediate)
	{
		SendImmediate((char*)temp.GetData(), temp.GetNumberOfBitsUsed(), LOW_PRIORITY, RELIABLE_ORDERED, orderingChannel, playerId, false, false, GetTime());
...
void RakPeer::ShiftIncomingTimestamp( unsigned char *data, PlayerID playerId ) const
{
	...
	BitStream timeBS( data, sizeof(RakNetTime), false);
...
void RakPeer::SecuredConnectionResponse( const PlayerID playerId )
{
	...
	if ( randomNumberExpirationTime < GetTime() )
		GenerateSYNCookieRandomNumber();
	...
	//SocketLayer::Instance()->SendTo( this, connectionSocket, ( char* ) connectionRequestResponse, 1 + sizeof( big::u32 ) + sizeof( RSA_BIT_SIZE ) + 20, playerId.binaryAddress, playerId.port );
	...
	SendImmediate(( char* ) connectionRequestResponse, (1 + sizeof( big::u32 ) + sizeof( RSA_BIT_SIZE ) + 20) *8, SYSTEM_PRIORITY, UNRELIABLE, 0, playerId, false, false, GetTime());
}
...
//	{
		...
		//SocketLayer::Instance()->SendTo( this, connectionSocket, reply, 1 + 20 + sizeof( RSA_BIT_SIZE ), playerId.binaryAddress, playerId.port );
		// All secure connection requests are unreliable because the entire process needs to be restarted if any part fails.
		// Connection requests are resent periodically
		SendImmediate((char*)reply, (1 + 20 + sizeof( RSA_BIT_SIZE )) * 8, SYSTEM_PRIORITY, UNRELIABLE, 0, remoteSystem->playerId, false, false, GetTime());
//	}
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void RakPeer::SendStaticDataInternal( const PlayerID target, bool performImmediate )
{
	...
	if (performImmediate)
	{
		if ( target == UNASSIGNED_PLAYER_ID )
			SendImmediate( (char*)reply.GetData(), reply.GetNumberOfBitsUsed(), SYSTEM_PRIORITY, RELIABLE, 0, target, true, false, GetTime() );
		else
			SendImmediate( (char*)reply.GetData(), reply.GetNumberOfBitsUsed(), SYSTEM_PRIORITY, RELIABLE, 0, target, false, false, GetTime() );
	}
	else
	{
		if ( target == UNASSIGNED_PLAYER_ID )
...
void RakPeer::PingInternal( const PlayerID target, bool performImmediate )
{
	...
	RakNetTime currentTime = GetTime();
...
bool RakPeer::SendImmediate( char *data, int numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool useCallerDataAllocation, RakNetTimeNS currentTime )
{
	...
	for (sendListIndex=0; sendListIndex < sendListSize; sendListIndex++)
	{
		...
		if ( outputTree )
		{
			BitStream bitStreamCopy( numberOfBytesUsed );
...
bool RakPeer::HandleBufferedRPC(BufferedCommandStruct *bcs, RakNetTime time)
{
	...
	for (sendListIndex=0; sendListIndex < (unsigned)sendListSize; sendListIndex++)
	{
		...
		BitStream outgoingBitStream((unsigned char *) writeTarget, dataBlockAllocationLength, false );
		outgoingBitStream.ResetWritePointer(); // Let us write at the start of the data block, rather than at the end
		...
		else
		{
			...
			StringCompressor::Instance()->EncodeString(uniqueID, 256, &outgoingBitStream);
		}
...
#endif
#ifdef _WIN32
void __stdcall RakNet::ProcessPortUnreachable( unsigned int binaryAddress, unsigned short port, RakPeer *rakPeer )
#else
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#ifdef _WIN32
void __stdcall RakNet::ProcessNetworkPacket( const unsigned int binaryAddress, const unsigned short port, const char *data, const int length, RakPeer *rakPeer )
#else
void RakNet::ProcessNetworkPacket( const unsigned int binaryAddress, const unsigned short port, const char *data, const int length, RakPeer *rakPeer )
#endif
{
	Packet *packet;
	PlayerID playerId;
	...
	if (rakPeer->IsBanned( rakPeer->PlayerIDToDottedIP( playerId ) ))
	{
		...
		SocketLayer::Instance()->SendTo( rakPeer, rakPeer->connectionSocket, (char*)&c, 2, playerId.binaryAddress, playerId.port );
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
	...
	else if (data[0] == ID_OPEN_CONNECTION_COOKIE && length == sizeof(unsigned char)*3)
	{
		for (i=0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length*8, playerId);
		...
		*(unsigned short*)&c[1] = *(unsigned short*)&data[1] ^ 0x6969; // Petarded [S04E06]

		unsigned i;
		for (i=0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketSend((char*)&c, 24, playerId);

		SocketLayer::Instance()->SendTo( rakPeer, rakPeer->connectionSocket, (char*)&c, 3, playerId.binaryAddress, playerId.port );
	...
	else
	{
			...
			|| (unsigned char)(data)[0] == ID_PING)	&& length == sizeof(unsigned char)+sizeof(RakNetTime) )
		{
				...
				rakPeer->AllowIncomingConnections() ) // Open connections with players
			{
				...
				//SocketLayer::Instance()->SendTo( rakPeer, connectionSocket, ( char* ) outBitStream.GetData(), outBitStream.GetNumberOfBytesUsed(), playerId.binaryAddress, playerId.port );
				...
				SocketLayer::Instance()->SendTo( rakPeer, rakPeer->connectionSocket, (const char*)outBitStream.GetData(), outBitStream.GetNumberOfBytesUsed(), (char*)rakPeer->PlayerIDToDottedIP(playerId) , playerId.port );
			}
		}
...
//	unsigned numberOfBytesUsed;
	...
	while (rcs)
	{
		...
		if (rcs->nextRequestTime < timeMS)
		{
			...
			SocketLayer::Instance()->SendTo( this, connectionSocket, (char*)&c, 3, rcs->playerId.binaryAddress, rcs->playerId.port );
		}
...
//						RakAssert(remoteSystem->connectMode==RemoteSystemStruct::HANDLING_CONNECTION_REQUEST);
							...
							playerId==myPlayerId) // local system connect
						{
							...
							BitStream inBitStream((unsigned char *) data, byteSize, false);
							PlayerID bsPlayerId;
							...
							//					BitStream nICS_BS( data, NewIncomingConnectionStruct_Size, false );
...
//						packet->playerIndex = ( PlayerIndex ) remoteSystemIndex;
					...
					else if ( (unsigned char) data[ 0 ] == 18 /* ID_RPC_MAPPING */ )
					{
						RakAssert(0);

						// Temporal check for SAMP.
					...
					else if ( (unsigned char)(data)[0] == ID_CONNECTION_REQUEST_ACCEPTED && byteSize == sizeof(unsigned char)+sizeof(unsigned int)+sizeof(unsigned short)+sizeof(PlayerIndex)+sizeof(unsigned int) )
					{
						...
						//	playerId.port = remotePort;
							...
							BitStream outBitStream(sizeof(unsigned char)+sizeof(unsigned int)+sizeof(unsigned short));
							...
							SendImmediate( (char*)outBitStream.GetData(), outBitStream.GetNumberOfBitsUsed(), SYSTEM_PRIORITY, RELIABLE, 0, playerId, false, false, GetTime() );
					...
					else if (data[0] == ID_AUTH_KEY && byteSize > (sizeof(unsigned char) + sizeof(unsigned char))) 
					{
							packet=AllocPacket(byteSize, data);
							packet->bitSize = bitSize;
							packet->playerId = playerId;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#ifdef _WIN32
unsigned __stdcall RakNet::UpdateNetworkLoop( LPVOID arguments )
#else
void* RakNet::UpdateNetworkLoop( void* arguments )
#endif
{
...
#ifdef _RAKNET_THREADSAFE
	#warning -- RakNet: _RAKNET_THREADSAFE defined.  Safe to use multiple threads on the same instance of RakPeer (Slow!). --
#else
	#warning -- RakNet: _RAKNET_THREADSAFE not defined.  Do NOT use multiple threads on the same instance of RakPeer (Fast!). --
#endif
