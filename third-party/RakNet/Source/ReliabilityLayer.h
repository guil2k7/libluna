...
constexpr int NUMBER_OF_ORDERED_STREAMS = 32; // 2^5

constexpr int RESEND_TREE_ORDER = 32;
...
public:
	...
	/// \param[in] MTUSize maximum datagram size
	/// \param[in] currentTime Current time, as per GetTime()
...
private:
	...
	unsigned GenerateDatagram( BitStream *output, int MTUSize, bool *reliableDataSent, RakNetTimeNS time, PlayerID playerId, DataStructures::List<PluginInterface*> &messageHandlerList );
	...
	int WriteToBitStreamFromInternalPacket( BitStream *bitStream, const InternalPacket *const internalPacket );

	/// Parse a bitstream and create an internal packet to represent this data
	InternalPacket* CreateInternalPacketFromBitStream( BitStream *bitStream, RakNetTimeNS time );
