...
BitStream::BitStream()
{
	...
	numberOfBitsAllocated = _RAK_BITSTREAM_STACK_ALLOCATION_SIZE * 8;
	readOffset = 0;
...
BitStream::BitStream( int initialBytesToAllocate )
{
	numberOfBitsUsed = 0;
	readOffset = 0;
	if (initialBytesToAllocate <= _RAK_BITSTREAM_STACK_ALLOCATION_SIZE)
	{
		data = ( unsigned char* ) stackData;
		numberOfBitsAllocated = _RAK_BITSTREAM_STACK_ALLOCATION_SIZE * 8;
	}
...
BitStream::BitStream( unsigned char* _data, unsigned int lengthInBytes, bool _copyData )
{
	...
	if ( copyData )
	{
		if ( lengthInBytes > 0 )
		{
			if (lengthInBytes < _RAK_BITSTREAM_STACK_ALLOCATION_SIZE)
			{
				data = ( unsigned char* ) stackData;
				numberOfBitsAllocated = _RAK_BITSTREAM_STACK_ALLOCATION_SIZE << 3;
			}
			else
			{
...
BitStream::~BitStream()
{
	if ( copyData && numberOfBitsAllocated > _RAK_BITSTREAM_STACK_ALLOCATION_SIZE << 3)
...
//		int newByteOffset = BITS_TO_BYTES( numberOfBitsAllocated );
		...
		{
			 if (amountToAllocate > _RAK_BITSTREAM_STACK_ALLOCATION_SIZE)
			 {
