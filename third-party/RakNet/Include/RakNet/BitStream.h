...
#define _RAK_BITSTREAM_STACK_ALLOCATION_SIZE 256
...
namespace RakNet
{
	...
	public:
		...
		/// There is no benefit to calling this, unless you know exactly how many bytes you need and it is greater than _RAK_BITSTREAM_STACK_ALLOCATION_SIZE.
		...
		/// Bidirectional serialize/deserialize any integral type to/from a bitstream.  Undefine _RAK_BITSTREAM_NATIVE_END if you need endian swapping.
		...
		/// Bidirectional serialize/deserialize any integral type to/from a bitstream.  Undefine _RAK_BITSTREAM_NATIVE_END if you need endian swapping.
		/// If you are not using _RAK_BITSTREAM_NATIVE_END the opposite is true for types larger than 1 byte
		...
		/// Write any integral type to a bitstream.  Undefine _RAK_BITSTREAM_NATIVE_END if you need endian swapping.
		/// \param[in] var The value to write
		template <class templateType>
			void Write(templateType var);

		template <class sizeType>
			void WriteString(std::string_view string, bool includeNullChar = true);
		...
		/// Read any integral type from a bitstream.  Define _RAK_BITSTREAM_NATIVE_END if you need endian swapping.
		/// \param[in] var The value to read
		template <class templateType>
			bool Read(templateType &var);

		template <class sizeType>
			bool ReadString(std::string &var, bool hasNullChar = true);
		...
		/// Read any integral type from a bitstream.  Undefine _RAK_BITSTREAM_NATIVE_END if you need endian swapping.
		/// For floating point, this is lossy, using 2 bytes for a float and 4 for a double.  The range must be between -1 and +1.
		/// For non-floating point, this is lossless, but only has benefit if you use less than half the range of the type
		/// If you are not using _RAK_BITSTREAM_NATIVE_END the opposite is true for types larger than 1 byte
		/// \param[in] var The value to read
	...
	private:
		...
		/// BitStreams that use less than _RAK_BITSTREAM_STACK_ALLOCATION_SIZE use the stack, rather than the heap to store data.  It switches over if _RAK_BITSTREAM_STACK_ALLOCATION_SIZE is exceeded
		unsigned char stackData[_RAK_BITSTREAM_STACK_ALLOCATION_SIZE];
	};
...
#pragma warning(disable:4127)   // conditional expression is constant
	...
	template <class sizeType>
		inline void BitStream::WriteString(std::string_view string, bool includeNullChar)
	{
		sizeType size = static_cast<sizeType>(string.length());
...
#pragma warning(disable:4244)   // '=' : conversion from 'unsigned long' to 'unsigned short', possible loss of data
	...
	template <class sizeType>
		inline bool BitStream::ReadString(std::string &var, bool hasNullChar)
	{
		sizeType size;
